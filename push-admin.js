...
// La función updateActiveRecurringNotificationCard se importa desde firebase-config.js
...
async function handleStartRecurringPush() {
  ...
  try {
    // Iniciar notificaciones push recurrentes (Firebase)
    // Pasar true para enviar una notificación inmediatamente
    startRecurringPushNotifications(title, message, imageUrl, interval, true);
    ...
    alert(`Notificaciones push recurrentes activadas. Se enviarán cada ${intervalText} a todos los dispositivos suscritos.`);
  } catch (error) {
    console.error('Error al iniciar notificaciones push recurrentes:', error);
    alert('Error al activar las notificaciones push recurrentes: ' + error.message);
  }
}
...
function handleStopRecurringPush() {
  try {
    // Detener notificaciones push recurrentes (Firebase)
    stopRecurringPushNotifications();

    alert('Notificaciones push recurrentes desactivadas.');
  } catch (error) {
    console.error('Error al detener notificaciones push recurrentes:', error);
    alert('Error al desactivar las notificaciones push recurrentes: ' + error.message);
  }
}
...
function initRecurringNotifications() {
  // Cargar configuración de notificaciones push recurrentes (Firebase)
  loadRecurringPushSettings();
}
